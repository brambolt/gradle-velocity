...
class PropertiesFileContextSpec extends Specification {
  ...
  def setup() {
    ...
    properties = new File(testProjectDir.newFolder('properties'), "${fileName}.properties")
    properties.text = '''
aProperty=aValue
belongsOrNot=belongs in properties
cProperty=cValue
'''
...
velocity {
  context(new File('${properties.absolutePath}'))
...
"""
  ...
  def 'can run task with properties context'() {
    when:
    ...
    then:
    null != result
    new File(outputDir, fileName).text.contains('The Deliverator belongs in properties')
  }
}
